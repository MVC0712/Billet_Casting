...
function makeTempTable() {
  var fileName = "SelTempTable.php";
  var sendData = {
    targetId: $("#selected__tr").find("td").eq(0).html(),
  };
  myAjax.myAjax(fileName, sendData);
  $("#homo_temp tbody").empty();
  ajaxReturnData.forEach(function (trVal) {
    var newTr = $("<tr>");
    Object.keys(trVal).forEach(function (tdVal) {
      if (tdVal == "staff_id") {
        $("<td>").append(staffOption(trVal[tdVal])).appendTo(newTr);
      }  else {
        $("<td>").html(trVal[tdVal]).appendTo(newTr);
      }
    });
    $(newTr).appendTo("#homo_temp tbody");
  });
};
function staffOption(seletedId) {
  let targetDom = $("<select>");

  fileName = "SelStaff.php";
  sendData = {
  };
  myAjax.myAjax(fileName, sendData);
...
function selStaff() {
  ...
  $("#temp_staff_id option").remove();
  $("#temp_staff_id").append($("<option>").val(0).html("NO"));
  ajaxReturnData.forEach(function(value) {
    $("#temp_staff_id").append(
      $("<option>").val(value["id"]).html(value["name"])
    );
  });
};
...
$(document).on("change", "#temp_staff_id", function() {
  if ($(this).val() != 0) {
    $(this).removeClass("no-input").addClass("complete-input");
  } else {
    $(this).removeClass("complete-input").addClass("no-input");
...
$(document).on("click", "#add_temp", function () {
  ...
  var sendData = {
    ...
    area3_1: $("#area3_1").val(),
    area3_2: $("#area3_2").val(),
    staff_id: $("#temp_staff_id").val(),
  };
  myAjax.myAjax(fileName, sendData);
  $("#time").val("").removeClass("complete-input").addClass("no-input");
  $("#area1_1").val("").removeClass("complete-input").addClass("no-input");
  $("#area1_2").val("").removeClass("complete-input").addClass("no-input");
  $("#area2").val("").removeClass("complete-input").addClass("no-input");
  $("#area3_1").val("").removeClass("complete-input").addClass("no-input");
  $("#area3_2").val("").removeClass("complete-input").addClass("no-input");
  $("#temp_staff_id").val(0).removeClass("complete-input").addClass("no-input");
...
function checkAddTempInput() {
  ...
  if ($("#temp_staff_id").val() == 0) {
    check = false;
  };
  if (check) {
    $("#add_temp").attr("disabled", false);
  } else {
    $("#add_temp").attr("disabled", true);
  } 
};